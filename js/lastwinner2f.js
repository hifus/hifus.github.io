...
function refreshBlock() {
    blockchain.getBlock(properties.LastBlock).then(function (block) {
        if (block && parseInt(block.number) === properties.LastBlock) {
            block.timestamp = parseInt(block.timestamp);
            var t = (new Date(block.timestamp * 1000)), h = t.getHours(), m = t.getMinutes(), s = t.getSeconds(),
                r = '<tr><td scope="row">' + properties.LastBlock + '</td><td>' + (h + ((m > 9) ? ':' : ':0') + m + ((s > 9) ? ':' : ':0') + s) + '</td><td>' + (block.timestamp - properties.LastBlockTime) + '</td></tr>';
            t = $('#blocks tbody');
            $(r).prependTo(t);
...
function getWinnerList() {
    if (properties.time < (new Date()).getTime()) {
        ...
        blockchain.getWinners(properties.histroy - 1).then(function (winner) {
            winner.round = properties.histroy;
            var f = function (block) {
                var t = parseInt(block.timestamp);
                var li = $('<li class="list-group-item p-2"><div><div style="float: left;">第' +
                    winner.round + '轮 第' + winner[3].toNumber() +
            ...
            if (winner[5].toNumber() === 0) {
                f({timestamp: 0});
            } else {
                blockchain.getBlock(winner[5]).then(f);
            }
                                ...
                                blockchain.getBlock(_blockNum - 1).then(function (block) {
                                    if (properties.LastBlock === 0) {
                                        if (block) {
                                            ...
                                            properties.LastBlock = parseInt(block.number) + 1;
                                            properties.LastBlockTime = parseInt(block.timestamp);
...
function refreshData() {
    blockchain.getInfo().then(function (promises) {
        if (promises[8].length === 42) {
            ...
            properties.cotoken = promises[7];
            properties.lastPlayer = promises[8].toLowerCase();

            n = promises[9];
            properties.autoRewards = n.mod(2).gt(0);
            properties.winnerCount = n.mod(properties.div24).div(2).floor().toNumber();

            n = n.div(properties.div24).floor();
            properties.fusRewares2 = n.mod(properties.div68);
            n = n.div(properties.div68).floor();
            properties.fusRewares = n.mod(properties.div68);
            n = n.div(properties.div68).div(properties.ether).toNumber();
            ...
            total = promises[5];
            n = total.mod(properties.div128).sub(total.div(properties.div128).floor()).div(properties.ether.div(10)).floor().mul(properties.fusRewares).div(properties.ether).toNumber();
            total = promises[6];
            total = total.mod(properties.div128).sub(total.div(properties.div128).floor()).div(properties.ether.div(10)).floor().mul(properties.fusRewares2).div(properties.ether).toNumber();
            $('#fus').text(n + total);
...
$(start(function (account) {
    properties.Contract = web3.eth.contract(abi).at(contractAddresses.LastWinner2f);
    ...
    return Promise.all([
        ...
        blockchain.getWinners = _promisfied[4];
        ...
        properties.fusRewares2 = web3.toBigNumber('0');
